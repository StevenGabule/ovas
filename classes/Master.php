...
Class Master extends DBConnection {
	...
	function submit_payment(){
		...
		$update = $this->conn->query("UPDATE appointment_list SET cash='{$cash}', userChange='{$change}', payment_status=1 where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Appointment Details has been successfully updated.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
